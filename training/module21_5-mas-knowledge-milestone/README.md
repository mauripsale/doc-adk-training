...
| **Cyclic Workflows** | 20 | `@node(rerun_on_resume=True)` with a Python `for`/`while` loop calling `ctx.run_node()` | Iterative refinement, self-correction, and "human-in-the-loop" review loops. |
